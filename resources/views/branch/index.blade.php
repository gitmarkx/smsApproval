                ...
                <tbody class="bg-white dark:bg-slate-800">
                    @foreach ($branches as $branch)
                        <tr onclick="window.location = '{{ route('branch.edit', $branch->id) }}'" class="hover:cursor-pointer hover:bg-slate-100">
                            <td class="border-b dark:border-slate-700 py-4 px-2 dark:text-slate-400">{{$branch->key}}</td>
                            <td class="border-b dark:border-slate-700 py-4 px-2 dark:text-slate-400">{{$branch->description}}</td>
                        </tr>
                    @endforeach
                </tbody>
            </table>
